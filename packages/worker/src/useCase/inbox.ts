...
import { Activity, FollowActivity, ActorResolver, ActivityStore, SignKeyResolver, acceptFollow } from '../domain'
  ...
  new: ({ now, signKeyResolver, actorResolver, activityStore }: Props): InboxUseCase => ({
    run: flow(
      ...
      TE.flatMap(({ activity, actor, cryptoKey }) =>
        pipe(
          activityStore.store(actor.inbox, acceptFollow(activity, now), cryptoKey, now),
